...
(() => {
  ...
  const titulo = params.get("titulo") || "Assistir filme";
  ...
  if (videoId) {
    ...
    if (iframe) {
      if (provider === "rede-canais") {
        if (!/^RCFServer\d+$/.test(server || "") || !/^[a-z0-9_-]+$/i.test(subfolder || "")) {
          const wrapper = document.querySelector(".player-wrapper");
          if (wrapper) wrapper.innerHTML = '<div class="player-error"><strong>Servidor deste filme ainda não foi configurado.</strong><a href="filmes.html">Voltar ao catálogo</a></div>';
          return;
        }
    ...
    if (tituloEl) tituloEl.innerText = "Filme não encontrado";
    const wrapper = document.querySelector(".player-wrapper");
    if(wrapper) wrapper.innerHTML = '<div class="player-error"><strong>Não foi possível abrir este filme.</strong><a href="filmes.html">Voltar ao catálogo</a></div>';
  }
